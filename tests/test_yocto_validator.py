...
def test_validate_artifacts_success(temp_artifacts_root):
    # Setup files
    Path(temp_artifacts_root, "zImage").touch()
    Path(temp_artifacts_root, "core-image-minimal.wic.gz").touch()
    Path(temp_artifacts_root, "core-image-minimal.manifest").touch()
    ...
    result = validate_artifacts(temp_artifacts_root, patterns)
...
def test_validate_artifacts_missing_some(temp_artifacts_root):
    # Only kernel exists
    Path(temp_artifacts_root, "zImage").touch()
...
def test_validate_artifacts_multiple_acceptable_names(temp_artifacts_root):
    # Both Image and zImage exist, should match 'kernel'
    Path(temp_artifacts_root, "Image").touch()
...
def test_validate_artifacts_empty_directory(temp_artifacts_root):
    patterns = {
        "kernel": ["zImage"]
    }
...
def test_validate_artifacts_recursive(temp_artifacts_root):
    # Setup nested structure
    os.makedirs(Path(temp_artifacts_root, "conf"))
    os.makedirs(Path(temp_artifacts_root, "meta-custom/conf"))
    
    Path(temp_artifacts_root, "conf/local.conf").touch()
    Path(temp_artifacts_root, "meta-custom/conf/layer.conf").touch()
    Path(temp_artifacts_root, "bzImage").touch()
...
def test_validate_artifacts_filename_match_in_subdir(temp_artifacts_root):
    # Setup nested file
    os.makedirs(Path(temp_artifacts_root, "nested/path"))
    Path(temp_artifacts_root, "nested/path/artifact.txt").touch()
